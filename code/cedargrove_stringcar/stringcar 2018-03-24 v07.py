...
# set up piezo output
piezo = pulseio.PWMOut(board.D0, duty_cycle=0, frequency=440, variable_frequency=True)
...
def trimpot(mnp=0, mxp=6.6):  # potentiometer position, voltage 0 to 6.6 volts
    return map_range(ctl.value, (0.05*(0xFFFF+1)), (0.95*(0xFFFF+1)), mnp, mxp)
...
def flash(color=0, count=1, on=0.5, off=0.5):  # blink indexed colors
    ...
    for i in range(count):
        ...
        if color == 5:
            dot[0] = [153, 0, 153]  # PONG MODE: violet
        if color == 6:
            dot[0] = [0, 0, 255]  # BOOMERANG MODE: blue
        if color == 7:
            dot[0] = [0, 0, 153]  # END OF BOOM: pale blue
        beep_on(784)  # E5
        beep_off(on)
        dot[0] = [0, 0, 0]  # off
        time.sleep(off)
